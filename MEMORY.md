...
- **Cron worker (MER-58)** : `CRON_SECRET` — secret partagé du worker d'audit durable. Vercel Cron l'envoie en `Authorization: Bearer $CRON_SECRET` ; le worker `/api/shopify/jobs/audit` refuse (401) sans lui et se désactive (503) s'il est absent. ⚠ **Action JB** : poser `CRON_SECRET` (Infisical + Vercel env), vérifier que le plan Vercel autorise la cadence cron (`vercel.json` = toutes les minutes) + `maxDuration` ≥ 60 s.
...
- **Bulk Operations Shopify (MER-26)** : (1) coût requête > 1000 = `MAX_COST_EXCEEDED` → bulk obligatoire ; (2) requête bulk = ≤ 5 connexions, profondeur ≤ 2 (notre `BULK_CATALOG_QUERY` : products→variants→metafields + products→metafields = 4 connexions, profondeur 2) ; (3) **1 bulk query / shop** à la fois → vérifier `currentBulkOperation(type: QUERY)` avant de lancer ; (4) le webhook `bulk_operations/finish` porte l'`admin_graphql_api_id` mais **pas l'url** du JSONL → la relire via `currentBulkOperation` ; (5) HMAC webhook = base64 sur le **corps brut** (`await req.text()` avant tout parse) ; (6) ingestion lourde dans le webhook = `after()` de `next/server` (ack < 5 s, traitement en tâche de fond). `read_metaobjects` est dans les scopes mais l'ingestion du **contenu** des metaobjects (vs leurs références en metafields) exige une requête par type → laissée en suivi (la couche raw_records gère déjà `resource_type='metaobject'`). Suivis MER-26 (non bloquants) : durabilité de l'**ingestion** en tâche de fond — MER-58 a livré la brique `background_jobs` (jobs durables Postgres + worker cron) pour l'**audit** ; migrer l'ingestion/normalisation vers cette même brique reste à faire ; rate-limit par org sur `/api/shopify/ingest` (la garde 1-bulk/shop borne déjà le travail).
- **Jobs durables / Vercel Cron (MER-58)** : (1) le claim DOIT passer par une RPC `FOR UPDATE SKIP LOCKED` (jamais un SELECT+UPDATE applicatif) sinon 2 ticks cron prennent le même job ; (2) lease via `locked_at` : un job `running` au lease périmé = worker mort → re-réclamé (reprise après crash) ; (3) `attempts` remis à 0 à chaque progrès (`saveJobProgress`) — sinon la continuation normale d'un gros catalogue épuise `max_attempts` ; (4) enqueue idempotent via index partiel unique `(connection_id, type) where status in (queued,running)` (webhook retenté par Shopify) ; (5) worker gardé par `CRON_SECRET` (Vercel Cron envoie `Authorization: Bearer`) ; `maxDuration` route ≥ budget temps (~50 s) → **exige le bon plan Vercel** ; (6) **gotcha Supabase** : `revoke ... from public` NE retire PAS l'EXECUTE d'`anon`/`authenticated` (accordé explicitement par les default privileges) → révoquer NOMMÉMENT `anon, authenticated` pour toute RPC service-role only (vaut aussi pour `persist_product_audit`, MER-57).
